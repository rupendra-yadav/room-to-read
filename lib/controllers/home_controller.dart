...
class HomeController extends GetxController {
  ...
  Future<void> fetchBookIssueCounts() async {
    try {
      ...
      await prefs.setInt(
        'cached_checkout_count_${currentUser.code}',
        checkoutCountValue,
      );

      print(
        '✅ Book counts updated - Issue: $issueCount, Checkout: $checkoutCountValue',
      );
    } catch (e) {
      ...
      try {
        ...
        if (currentUser != null) {
          final prefs = await SharedPreferences.getInstance();
          final cachedReturn = prefs.getInt(
            'cached_return_count_${currentUser.code}',
          );
          final cachedCheckout = prefs.getInt(
            'cached_checkout_count_${currentUser.code}',
          );
          if (cachedReturn != null) returnCount.value = cachedReturn;
          if (cachedCheckout != null) checkoutCount.value = cachedCheckout;
          print(
            '📦 Restored counts from cache - Return: $cachedReturn, Checkout: $cachedCheckout',
          );
        }
      } catch (cacheError) {
